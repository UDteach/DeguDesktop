...
func TestParseUpdateApplyArgsRequiresSafeCleanupDir(t *testing.T) {
	...
	opts, err := parseUpdateApplyArgs([]string{
		"--source", filepath.Join(cleanupDir, "payload", "DeguDesktop.exe"),
		"--target", filepath.Join(targetDir, "DeguDesktop.exe"),
		"--parent-pid", "1234",
	...
	if _, err := parseUpdateApplyArgs([]string{
		"--source", filepath.Join(t.TempDir(), "DeguDesktop.exe"),
		"--target", filepath.Join(targetDir, "DeguDesktop.exe"),
		"--cleanup-dir", cleanupDir,
	}); err == nil {
	...
	if _, err := parseUpdateApplyArgs([]string{
		"--source", filepath.Join(cleanupDir, "payload", "DeguDesktop.exe"),
		"--target", filepath.Join(cleanupDir, "installed", "DeguDesktop.exe"),
		"--cleanup-dir", cleanupDir,
	}); err == nil {
	...
	if _, err := parseUpdateApplyArgs([]string{
		"--source", filepath.Join(cleanupDir, "payload", "DeguDesktop.exe"),
		"--target", filepath.Join(targetDir, "renamed.exe"),
		"--cleanup-dir", cleanupDir,
	}); err == nil {
...
func TestExtractUpdateExeUsesFixedPayloadPath(t *testing.T) {
	...
	w, err := zw.Create("release/nested/DeguDesktop.exe")
	if err != nil {
	...
	want := filepath.Join(tmpDir, "payload", "DeguDesktop.exe")
	if exePath != want {
...
func TestUpdaterCommandsInvokeAppExeDirectly(t *testing.T) {
	...
	sourceExe := filepath.Join(cleanupDir, "payload", "DeguDesktop.exe")
	targetExe := filepath.Join(t.TempDir(), "DeguDesktop.exe")
	helperExe := filepath.Join(cleanupDir, "helper", "DeguDesktop.exe")
...
func TestReleaseWorkflowPowerShellBlocksParseAfterGitHubSubstitution(t *testing.T) {
	...
	for stepName, script := range scriptByStep {
		t.Run(stepName, func(t *testing.T) {
			script = strings.ReplaceAll(script, "${{ github.ref_name }}", "v0.1.13")
...
func TestReleaseWorkflowPackageIncludesSecurityManifestAndHashes(t *testing.T) {
	...
	for _, want := range []string{
		"SECURITY.txt",
		"SHA256SUMS.txt",
		"DeguDesktop-windows-amd64.zip/DeguDesktop.exe",
		"DeguDesktop-windows-386.zip/DeguDesktop.exe",
...
func TestReleaseWorkflowPublishesWindowsTrustAssets(t *testing.T) {
	workflowPath := filepath.Join("..", "..", ".github", "workflows", "release.yml")
	...
	for _, want := range []string{
		...
		"go run github.com/tc-hib/go-winres@v0.3.1 make --arch amd64,386 --out cmd/degu/rsrc",
...
func extractWorkflowRunBlock(t *testing.T, workflowPath, stepName string) string {
	...
	if strings.TrimSpace(lines[runIndex]) != "run: |" {
		return strings.TrimPrefix(strings.TrimSpace(lines[runIndex]), "run: ")
	}
	contentIndent := -1
...
func assertPowerShellParses(t *testing.T, script string) {
	...
	tempDir := t.TempDir()
	scriptPath := filepath.Join(tempDir, "script-under-test.ps1")
	if err := os.WriteFile(scriptPath, []byte(script), 0o600); err != nil {
		t.Fatalf("write script under test: %v", err)
	}
	parserPath := filepath.Join(tempDir, "parse-script.ps1")
	parser := `
...
`
	...
	cmd := exec.Command(powershell, "-NoProfile", "-NonInteractive", "-File", parserPath, scriptPath)
